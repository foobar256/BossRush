...
@export var config_path: String = "user://boss_config.cfg"
...
func _process(_delta: float) -> void:
	if Input.is_action_just_pressed(toggle_action):
		...
		if visible:
			_refresh_boss()
		_update_cursor_state()
	if visible and _bosses.is_empty():
		_refresh_boss()
...
func _refresh_boss() -> void:
	...
	if _boss == null:
		_status_label.text = "Boss: none"
		_clear_controls()
		return
	_status_label.text = "Bosses: %d" % _bosses.size()
	_apply_saved_config()
	_build_controls()
...
func _apply_slider_range(slider: HSlider, prop: Dictionary, value: float) -> void:
	...
	if prop_name == "current_health":
		...
		slider.max_value = max(1.0, _get_max_boss_health())
		slider.step = 1.0
		return
...
func _write_config(values: Dictionary) -> void:
	...
	if err != OK:
		push_warning("Failed to write boss config: %s" % config_path)
